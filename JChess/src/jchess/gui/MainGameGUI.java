...
public class MainGameGUI extends Application implements Runnable{
    private Game game;
    public MainGameGUI(Game game){
        this.game = game;
    }
    private JChessClient client;
                ...
                for (int row =1; row<=8;row++) {
                    ...
                    rect.setOnMouseClicked(mouseEvent -> {
                        if(!this.move.isEmpty()){
                            int x = game.find(this.move.substring(0,1));
                            int y = Integer.parseInt(this.move.substring(1,2))-1;
                            if(game.getTable()[y][x] != null){
                                if(game.getTable()[y][x].getSide().equals(client.getSide())){ // Check if the piece is actually of the player
                                    move(rect.getId());
                                }else{
                                    System.out.println("Cannot move opponent piece");
                                    this.move = "";
                                    repaint();
                                }
                            }else{
    ...
    public void removePieceFromBoard(String action){
